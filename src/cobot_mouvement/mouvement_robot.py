...
# Position allowing the cobot to see both trays          
if arm.error_code == 0 and not params['quit']:
    code = arm.set_servo_angle(angle=[-56.3, -7.5, -119, 0.6, 124.4,-23], speed=params['angle_speed'], mvacc=params['angle_acc'], wait=False, radius=-1.0)
    if code != 0:
        params['quit'] = True
...
# Movement for the first tray
if id==1:
    arm.load_trajectory('test_bas.traj')
    arm.playback_trajectory()
    arm.load_trajectory('pose.traj')
    arm.playback_trajectory()
# Movement for the second tray
elif id == 2:
    arm.load_trajectory('test_haut.traj')
    arm.playback_trajectory()
    arm.load_trajectory('pose.traj')
    arm.playback_trajectory()
